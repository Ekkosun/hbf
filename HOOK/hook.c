...
FILE *fopen(const char *filename, const char *mode)
{
  ...
  if (ERRNO)
    goto init_failed;
  if (stream && filename && FILE_NAME && strstr(FILE_NAME, filename) || strstr(filename, FILE_NAME))
  {
...
int open(const char *filename, int flags, ...)
{
  ...
  if (ERRNO)
    goto init_failed;
  if (fd > 0 && filename && FILE_NAME && strstr(FILE_NAME, filename) || strstr(filename, FILE_NAME))
  {
    TEST_FILE = fd;
  }
init_failed:
...
size_t fread(void *ptr, size_t size, size_t nmemb, FILE *stream)
{
  ...
  if (ERRNO)
    goto init_failed;
  if (SHM_READ_INFO && stream && is_same(fileno(stream), TEST_FILE) == 1)
  {
...
ssize_t read(int fd, void *buf, size_t count)
{
  ...
  if (ERRNO)
    goto init_failed;
  if (fd > 0 && SHM_READ_INFO && is_same(fd, TEST_FILE) == 1)
  {
...
int fclose(FILE *stream)
{
  if (ERRNO)
    goto init_failed;
  if (stream && is_same(TEST_FILE, fileno(stream)) == 1)
  {
...
int close(int fd)
{
  if (ERRNO)
    goto init_failed;
  if (fd > 0 && is_same(TEST_FILE, fd) == 1)
  {
